...
public readonly partial struct SolarSystemRenderShader(
    ...
    float isBlueStar,
    int secondaryStarEnabled,
    float secondaryOrbitRadius,
    float secondaryStarRadius,
    float secondaryPhase,
    Float3 secondaryStarColor,
    Float3 secondaryStarHotColor,
    Float3 secondaryStarWhiteColor,
    ...
    int debrisQuality) : IComputeShader
{
    public void Execute()
    {
        ...
        if (viewMode == 0 && showHabitableZone != 0)
            color += HabitableZone(uv, HabitableRenderRadius());
        ...
        if (secondaryStarEnabled != 0 && secondaryOrbitRadius > 0f && showOrbits != 0)
            color += SecondaryOrbitLine2D(uv);
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            color = DrawMoon(color, uv, projected.XY, VisualRadius(data.Z) * projected.Z, extra.Z, data.W, i == selectedMoonIndex);
        }

        color += DrawSecondaryStar2D(uv);
        color += DrawStar(uv);
    ...
    private Float3 RenderRaycast3D(Float3 color, Float2 uv)
    {
        ...
        Float3 rayDir = Hlsl.Normalize(forward + right * uv.X * 0.50f - up * uv.Y * 0.50f);

        color = Skybox(rayDir);
        color += OrbitsRaycast3D(camPos, rayDir);
        color += DrawStarGlowRaycast(camPos, rayDir, right, up);
        color += DrawSecondaryStarGlowRaycast(camPos, rayDir);
        ...
        if (secondaryStarEnabled != 0)
        {
            Float3 secondaryCenter = SecondaryStarPosition();
            float secondaryT = RaySphere(camPos, rayDir, secondaryCenter, secondaryStarRadius);
            if (secondaryT > 0f && secondaryT < bestT)
            {
                Float3 hit = camPos + rayDir * secondaryT;
                Float3 n = Hlsl.Normalize((hit - secondaryCenter) / secondaryStarRadius);
                Float3 sampleN = RotateY3(n, time * 0.16f + seed + secondaryPhase);
                float rim = Hlsl.Pow(1f - Hlsl.Saturate(Hlsl.Dot(n, -rayDir)), 3.2f);
                bestColor = SecondaryPhotosphere3D(sampleN) + secondaryStarColor * rim * 1.85f;
                bestT = secondaryT;
                hasHit = true;
            }
        }
        ...
        for (int i = 0; i < 9; i++)
        {
            ...
            float markerRadius = realismMode != 0 ? Hlsl.Max(radius * 8.0f, 0.060f) : radius;
            float markerGlow = Hlsl.Exp(-(closest * closest) / (markerRadius * markerRadius * 1.75f + 1e-5f));
            Float3 markerColor = PlanetAlbedo(Hlsl.Normalize(center + new Float3(0.2f, 0.6f, 0.1f)), (int)data.W, extra.Z, extra.W);
            color += markerColor * markerGlow * (realismMode != 0 ? 0.24f : 0.0f);
            if (i == selectedIndex)
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            float t = RaySphere(camPos, rayDir, center, radius);
            float closest = RayPointDistance(camPos, rayDir, center);
            float markerRadius = realismMode != 0 ? Hlsl.Max(radius * 7.0f, 0.044f) : radius;
    ...
    {
        float z = Hlsl.Max(zoom, 0.08f);
        float dist = realismMode != 0
            ? 12.0f / Hlsl.Pow(z, 1.06f) + 0.20f
            : 2.85f / Hlsl.Pow(z, 1.04f) + 0.075f;
    ...
    private Float3 SecondaryStarPosition()
    {
        float baseOrbit = Hlsl.Abs(secondaryOrbitRadius);
        float orbit = OrbitRadius(baseOrbit);
        float speed = 0.060f / Hlsl.Sqrt(Hlsl.Max(baseOrbit, 0.18f));
        float phase = secondaryPhase + orbitTime * speed;
        return new Float3(
            Hlsl.Cos(phase) * orbit,
            0f,
            Hlsl.Sin(phase) * orbit * 0.92f);
    }
    ...
    private Float3 OrbitsRaycast3D(Float3 rayOrigin, Float3 rayDir)
    {
        ...
        if (showHabitableZone != 0)
        {
            float hz = HabitableRenderRadius();
            float hr = Hlsl.Sqrt(p.X * p.X + p.Z * p.Z + 1e-6f);
            sum += HabitableZoneColor(hr, hz, 0.16f);
        }
        if (secondaryStarEnabled != 0 && secondaryOrbitRadius > 0f)
            sum += SecondaryOrbitLineAtPlane(p);
    ...
    private Float3 DrawSecondaryStar2D(Float2 uv)
    {
        if (secondaryStarEnabled == 0)
            return Float3.Zero;

        Float3 center = SecondaryStarPosition();
        Float2 local = uv - center.XZ;
        float r = Hlsl.Sqrt(Hlsl.Dot(local, local) + 1e-6f);
        float radius = secondaryStarRadius;
        float angle = Hlsl.Atan2(local.Y, local.X) + time * 0.04f + secondaryPhase;
        float outside = Hlsl.Saturate((r - radius) / 0.12f);
        float near = Hlsl.Saturate(1f - outside);
        float angular = Fbm(new Float2(Hlsl.Cos(angle) * 3.2f + time * 0.08f, Hlsl.Sin(angle) * 3.0f - time * 0.12f + r * 3.8f), 5);
        float halo = Hlsl.Exp(-Hlsl.Max(r - radius, 0f) * 14.0f);
        float rim = Hlsl.Exp(-Hlsl.Abs(r - radius) * 65f);
        Float3 color = Hlsl.Lerp(secondaryStarColor, secondaryStarHotColor, 0.65f + 0.35f * angular) * halo * (0.42f + angular * 0.95f) * near;
        color += secondaryStarHotColor * rim * 1.55f;
        color += SecondaryMagneticLoops2D(local, radius);
        if (r <= radius)
        {
            float z = Hlsl.Sqrt(Hlsl.Max(radius * radius - r * r, 0f));
            Float3 n = Hlsl.Normalize(new Float3(local.X, -local.Y, z));
            Float3 sampleN = RotateY3(n, time * 0.18f + seed + secondaryPhase);
            float view = Hlsl.Saturate(n.Z);
            float limb = Hlsl.Pow(1f - view, 2.25f);
            float rim2 = Hlsl.Pow(1f - view, 8f);
            Float3 plasma = SecondaryPhotosphere3D(sampleN);
            float sparks = Hlsl.Pow(Fbm(sampleN.YZ * 24f + seed + secondaryPhase + time * 0.06f, 3), 7f);
            plasma += secondaryStarWhiteColor * sparks * 1.5f;
            plasma += secondaryStarHotColor * rim2 * 2.3f;
            plasma += secondaryStarColor * limb * 0.65f;
            float edge = Hlsl.Saturate((radius - r) / 0.006f);
            color = Hlsl.Lerp(color, plasma, edge);
            color += secondaryStarHotColor * rim2 * 0.75f;
        }

        return color;
    }

    private Float3 SecondaryPhotosphere3D(Float3 n)
    {
        Float3 flowN = Hlsl.Normalize(n + new Float3(Hlsl.Sin(time * 0.050f + seed + secondaryPhase), Hlsl.Cos(time * 0.041f), Hlsl.Sin(time * 0.035f - seed)) * 0.085f);
        float warpA = SeamlessNoise(flowN, 4.2f, seed + secondaryPhase, 5);
        float warpB = SeamlessNoise(flowN, 7.0f, seed * 1.37f + secondaryPhase - time * 0.04f, 4);
        Float3 q = Hlsl.Normalize(flowN + new Float3(warpA - 0.5f, warpB - 0.5f, warpA - warpB) * 0.62f);
        float convection = SeamlessNoise(q, 8.0f, seed + secondaryPhase + time * 0.025f, 5);
        float fine = SeamlessNoise(q, 30.0f + qualityLevel * 12f, seed * 0.7f + secondaryPhase - time * 0.05f, qualityLevel == 0 ? 4 : 5);
        float cellular = Hlsl.Sin((q.X + fine * 0.25f) * 13.0f) * Hlsl.Cos((q.Y - convection * 0.30f) * 15.0f) * Hlsl.Sin((q.Z + warpB * 0.20f) * 11.0f);
        float vein = Hlsl.Pow(Hlsl.Saturate(1f - Hlsl.Abs(cellular)), 2.4f);
        float darkCrack = Hlsl.Pow(1f - Hlsl.Abs(fine * 2f - 1f), 4.6f);
        float heat = 0.18f + convection * 0.48f + vein * 0.56f - darkCrack * 0.42f;
        heat += Hlsl.Pow(SeamlessNoise(q, 72f, seed + secondaryPhase + 9.1f, qualityLevel == 2 ? 4 : 3), 6f) * 0.45f;
        heat = Hlsl.Saturate(heat);
        Float3 c = heat < 0.46f
            ? Hlsl.Lerp(secondaryStarColor * 0.32f, secondaryStarColor, heat / 0.46f)
            : heat < 0.82f
                ? Hlsl.Lerp(secondaryStarColor, secondaryStarHotColor, (heat - 0.46f) / 0.36f)
                : Hlsl.Lerp(secondaryStarHotColor, secondaryStarWhiteColor, (heat - 0.82f) / 0.18f);
        return c * (1.55f + heat * 2.65f);
    }

    private Float3 DrawSecondaryStarGlowRaycast(Float3 rayOrigin, Float3 rayDir)
    {
        if (secondaryStarEnabled == 0)
            return Float3.Zero;

        Float3 center = SecondaryStarPosition();
        float d = RayPointDistance(rayOrigin, rayDir, center);
        float glow = Hlsl.Exp(-d * 10.5f) * 0.82f + Hlsl.Exp(-d * 31.0f) * 0.70f;
        float rim = Hlsl.Exp(-Hlsl.Abs(d - secondaryStarRadius) * 50f) * 1.15f;
        return Hlsl.Lerp(secondaryStarColor, secondaryStarHotColor, 0.58f) * glow +
            secondaryStarHotColor * rim;
    }

    private Float3 SecondaryMagneticLoops2D(Float2 uv, float radius)
    {
        if (qualityLevel <= 0)
            return Float3.Zero;
        ...
        for (int i = 0; i < 4; i++)
        {
            float loopSeed = seed + secondaryPhase + i * 19.17f;
            float h = Hash(loopSeed);
            float phase = Hlsl.Frac(time * (0.070f + Hash(loopSeed + 12.41f) * 0.042f) + Hash(loopSeed + 33.7f));
            ...
            float span = (0.28f + Hash(loopSeed + 9.13f) * 0.42f) * (0.45f + 0.55f * grow);
            float lift = (0.18f + Hash(loopSeed + 5.77f) * 0.30f) * life;
            ...
            float breakup = Fbm(new Float2(t * 11.0f + loopSeed, time * 0.85f + i * 1.7f), 4);
            ...
            sum += Hlsl.Lerp(secondaryStarHotColor, secondaryStarWhiteColor, 0.30f + 0.35f * h) * mask * outside * 0.46f;
        }
    ...
    {
        float spinDirection = Hash(moonSeed + 2.7f) > 0.34f ? 1f : -1f;
        float spinSpeed = 0.24f + Hash(moonSeed + 5.9f) * 0.38f;
        n = RotateY3(n, moonSeed * 0.11f + time * spinSpeed * spinDirection);
    ...
    private Float3 DrawMoon(Float3 baseColor, Float2 uv, Float2 center, float radius, float moonSeed, float icy, bool selected)
    {
        Float2 d = uv - center;
        float r2 = Hlsl.Dot(d, d);
        ...
        float z = Hlsl.Sqrt(Hlsl.Max(radius * radius - r2, 0f));
        Float3 n = Hlsl.Normalize(new Float3(d.X, -d.Y, z));
        Float3 albedo = MoonAlbedo(n, moonSeed, icy);
        Float3 lightDir = Hlsl.Normalize(new Float3(-center.X, center.Y, 0.42f));
        float diffuse = Hlsl.Saturate(Hlsl.Dot(n, lightDir));
    ...
    {
        float r = Hlsl.Sqrt(Hlsl.Dot(uv, uv) + 1e-6f);
        return HabitableZoneColor(r, hz, 0.060f);
    }

    private static Float3 HabitableZoneColor(float radius, float hz, float intensity)
    {
        float width = Hlsl.Max(hz * 0.28f, 0.060f);
        float dist = Hlsl.Abs(radius - hz);
        float edgeFade = Hlsl.Saturate((width - dist) / Hlsl.Max(width * 0.18f, 0.012f));
        float green = Hlsl.Exp(-(dist * dist) / Hlsl.Max(width * width * 0.18f, 1e-5f));
        float yellow = Hlsl.Exp(-((dist - width * 0.50f) * (dist - width * 0.50f)) / Hlsl.Max(width * width * 0.070f, 1e-5f));
        float red = Hlsl.Pow(Hlsl.Saturate(dist / width), 2.4f);
        Float3 zoneColor =
            new Float3(0.95f, 0.18f, 0.10f) * red +
            new Float3(0.95f, 0.76f, 0.18f) * yellow * 0.72f +
            new Float3(0.18f, 0.78f, 0.34f) * green;
        return zoneColor * edgeFade * intensity;
    }

    private Float3 SecondaryOrbitLine2D(Float2 uv)
    {
        float orbit = OrbitRadius(Hlsl.Abs(secondaryOrbitRadius));
        float e = 0.92f;
        float d = Hlsl.Abs(Hlsl.Sqrt((uv.X * uv.X) / (orbit * orbit) + (uv.Y * uv.Y) / (orbit * orbit * e * e) + 1e-6f) - 1f);
        float line = Hlsl.Exp(-(d * d) / 0.000040f);
        return Hlsl.Lerp(secondaryStarColor, new Float3(0.40f, 0.82f, 1.0f), 0.35f) * line * 0.20f;
    }

    private Float3 SecondaryOrbitLineAtPlane(Float3 p)
    {
        float orbit = OrbitRadius(Hlsl.Abs(secondaryOrbitRadius));
        float e = 0.92f;
        float d = Hlsl.Abs(Hlsl.Sqrt((p.X * p.X) / (orbit * orbit) + (p.Z * p.Z) / (orbit * orbit * e * e) + 1e-6f) - 1f);
        float line = Hlsl.Exp(-(d * d) / 0.000052f);
        return Hlsl.Lerp(secondaryStarColor, new Float3(0.40f, 0.82f, 1.0f), 0.35f) * line * 0.17f;
    }
    ...
    private Float3 Skybox(Float3 dir)
    {
        Float3 n = Hlsl.Normalize(dir);
        float pole = Hlsl.Abs(n.Y);
        Float3 bg = Hlsl.Lerp(new Float3(0.006f, 0.009f, 0.017f), new Float3(0.018f, 0.026f, 0.040f), Hlsl.Saturate(0.35f + pole * 0.45f));
        float starCore = StarField3D(n, 145f, 0.9966f, 0.050f);
        float bright = StarField3D(n, 64f, 0.9984f, 0.080f) * 1.9f;
        float band = Hlsl.Exp(-(n.Y * n.Y) / 0.020f);
        float dust = Fbm(new Float2(n.X * 7.0f + n.Z * 2.8f + seed, n.Y * 16.0f + n.Z * 4.2f - seed * 0.3f), 4);
        Float3 milky = new Float3(0.42f, 0.48f, 0.58f) * band * (0.030f + dust * 0.050f);
        Float3 tint = Hlsl.Lerp(new Float3(0.70f, 0.82f, 1.0f), new Float3(1.0f, 0.88f, 0.62f), Hash(n.X * 17.0f + n.Y * 31.0f + n.Z * 47.0f + seed));
        return bg + milky + tint * (starCore * 0.75f + bright);
    }

    private static float StarField3D(Float3 dir, float density, float threshold, float width)
    {
        Float3 p = dir * density;
        Float3 cell = Hlsl.Floor(p);
        Float3 f = p - cell;
        float rnd = Hash3(cell);
        Float3 sp = new Float3(Hash3(cell + 17.1f), Hash3(cell + 37.7f), Hash3(cell + 71.3f));
        float d = Hlsl.Sqrt(Hlsl.Dot(f - sp, f - sp));
        return rnd > threshold ? Hlsl.Exp(-(d * d) / (width * width)) : 0f;
    }
    ...
    private static float Hash3(Float3 p)
        => Hlsl.Frac(Hlsl.Sin(Hlsl.Dot(p, new Float3(127.1f, 311.7f, 74.7f))) * 43758.5453f);
    ...
    private float VisualRadius(float renderRadius)
        => realismMode != 0 ? Hlsl.Max(renderRadius * 0.42f, 0.0065f) : renderRadius;

    private float HabitableRenderRadius()
    {
        float hz = Hlsl.Clamp(0.46f + Hlsl.Pow(Hlsl.Max(habitableZoneAu, 0.03f), 0.86f) * 1.18f, 0.28f, 8.20f);
