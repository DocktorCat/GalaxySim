...
public sealed class SimulationHost : IDisposable
{
    ...
    public GalaxyPresetType CurrentPreset { get; private set; } = GalaxyPresetType.MilkyWay;
    public GalaxySearchIndex SearchIndex { get; private set; } = GalaxySearchIndex.Empty;
    ...
    public void ApplyPreset(GalaxyPresetType presetType, bool regenerateCurrentScenario = true)
    {
        ...
        if (regenerateCurrentScenario)
            LoadScenario(_currentScenario, GalaxySeed);
    }
    ...
    public void LoadScenario(Scenario scenario, int galaxySeed = 0)
    {
        ...
        if (galaxySeed != 0)
        {
            for (int i = 0; i < placements.Length; i++)
                placements[i].Seed = unchecked(placements[i].Seed + galaxySeed + i * 9973);
        }
        ...
        SearchIndex = new GalaxySearchIndex(GalaxySeed, pos, vel);
    ...
    public IReadOnlyList<StarSearchEntry> SearchStars(SearchSettingsState settings, int maxResults = 80)
        => SearchIndex.Search(settings, maxResults);

    public StarSearchEntry? GetSearchEntry(int starIndex)
        => SearchIndex.GetEntry(starIndex);
    ...
    public SelectedStarInfo? GetStarByIndex(int starIndex)
    {
        if (starIndex < 0 || starIndex >= ParticleCount)
            return null;

        _device.For(1, new CopyPickedStarShader(_positions, _velocities, _pickSelected, starIndex, ParticleCount));
        _pickSelected.CopyTo(_pickSelectedCpu);
